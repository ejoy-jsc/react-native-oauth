...
class OAuthManagerModule extends ReactContextBaseJavaModule {
  ...
  @ReactMethod
  public void refreshauthen(final String providerName, @Nullable final ReadableMap params, final Callback callback) {
    Log.e(TAG, "refreshauthen is not implemented for android yet !!!");
    WritableMap err = Arguments.createMap();
    err.putString("status", "error");
    err.putString("msg", "refreshauthen is not implemented for android yet !!!");
    callback.invoke(err);
    return;
  }
  ...
  private WritableMap accessTokenResponse(final String providerName, final HashMap<String, Object> cfg,
    ...
    try {
      ...
      /*
       * failed to parse as JSON, so turn it into a HashMap which looks like the one
       * we'd get back from the JSON parser, so the rest of the code continues
       * unchanged.
       */
